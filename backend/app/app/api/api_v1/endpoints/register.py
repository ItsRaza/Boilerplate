...
def sendEmail(code):
    try:
        ...
        if cfg.emailMetaData:
            message = 'Subject: {}\n\n{}'.format(
                cfg.emailMetaData["subject"], cfg.emailMetaData["text"] + str(code))
            server = smtplib.SMTP(
                cfg.emailMetaData["smtp-server"], cfg.emailMetaData["port"])
            server.ehlo()
            server.starttls()
            server.login(cfg.emailMetaData["username"], cfg.emailMetaData["password"])
            server.sendmail(cfg.emailMetaData["username"],
...
def validateCode(returnToken, returnEmail=""):
    try:
        tokenMetaData = jwt.decode(trimToken(returnToken),
                                   cfg.jwtSecret, algorithms=['HS256'])
        # Validate JWT
        if tokenMetaData["exp"] < int(time.time()):
            ...
            if tokenMetaData["issuedFor"] != returnEmail:
                print(
                    "Failed to validate returning token. Error ==> Code not associated with email address specified")
                return False
            return True
        else:
            return True
...
if __name__ == "__main__":
    # Should return the code (base64 encode format)
    code = generateCode("razaabbas768@gmail.com")
    print(sendEmail(code))  # Should return True

    print(validateCode(code))  # Should return True
